...
typedef struct sCfState
{
    float phi;    // roll  [rad]
    float theta;  // pitch [rad]
    float psi;    // yaw   [rad] — updated by mag if available
} sCfState;
typedef sCfState CfState;

void cfInit(sCfState& s, float phi0, float theta0, float psi0);
...
void cfUpdate(sCfState& s, float gx, float gy, float gz, float ax, float ay, float az, float dt, float alpha);

// Update with magnetometer [uT] for yaw correction (call after cfUpdate)
void cfUpdateYaw(sCfState& s, float mx, float my, float mz, float alpha_yaw);

EulerAngle cfGetEuler(const sCfState& s);