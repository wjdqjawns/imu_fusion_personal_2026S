...
본 부록에서는 쿼터니언의 기본 대수 연산과 성질을 정리한다.
4장 및 5장의 수식 유도에 필요한 기초 내용을 포함한다.
 
% ------------------------------------------------------------
\subsection*{A.1 기본 정의}
 
쿼터니언 $\mathbf{q} \in \mathbb{H}$는 실수 스칼라 성분 $q_0$와
허수 벡터 성분 $\mathbf{q}_v = [q_1, q_2, q_3]^\top$으로 구성된다:
...
두 쿼터니언 $\mathbf{p} = [p_0, \mathbf{p}_v]^\top$,
$\mathbf{q} = [q_0, \mathbf{q}_v]^\top$의 곱:
