...
_MSG_PAYLOAD_LEN = 42           # seq(2)+time_us(4)+floats(36)
...
def _build_frame(seq: int, t_us: int, ax, ay, az, gx, gy, gz, mx, my, mz) -> bytes:
    header  = struct.pack('<HHH', _FRAME_HEADER, _MSG_PAYLOAD_LEN, seq)
    payload = struct.pack('<Ifffffffff', t_us, ax, ay, az, gx, gy, gz, mx, my, mz)
    body    = header + payload          # CRC 범위 = header 부터 mz 까지
...
class MockSerialBinary:
    ...
    지터 시뮬레이션: time_us 에 가우시안 노이즈(±JITTER_US_SIGMA) 추가.
    """
    TARGET_HZ       = 100.0
    ...
    def _flush_pending(self):
        ...
        while self._next_t <= now:
            ...
            self._buf += _build_frame(
                ...
                random.gauss(-1.0,  0.02),   # az  [g]  ≈ -1g 정치 상태
