...
Description:
  Madgwick Filter — gradient descent 기반 AHRS 필터
    ...
    purpose:
        오차 함수의 기울기를 계산하여 자세를 보정.
        beta = ω_err · √(3/4): 자이로 측정 오차 가중치.
...
class MadgwickFilter(AhrsFilter):
    ...
    def update(self, gyro: np.ndarray, accel: np.ndarray,
        ...
        if a_norm > 1e-6:
            ...
            # â = -accel_normalized (gravity direction in body frame)
            ax, ay, az = -accel / a_norm

            # Objective: f_g = q* ⊗ [0,0,1,0] ⊗ q - â
